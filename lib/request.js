...
/**
 * Start the timer for a particular handler. Restify already does this for
 * parent handlers that are registered as part of a route, however, if you have
 * nested handlers, then you will want to start the timers for them using this.
 * Note however that you currently do not get DTrace probes for user registered
 * timers.
 *
 * If you started timeing a user registered handler, you MUST invoke endTimer
 * when the handler is finished.
 */
Request.prototype.startTimer = function startTimer(name, i) {
    var self = this;
    self.lastHandler = {
        name: name || 'handler-' + i,
        time: process.hrtime(),
    ...
    self.timers.push(self.lastHandler);
};

/**
 * If you have parallel nested handlers, you can call endTimer each time a
 * parallel nested handler has finished. You must invoke the parent endTimer
 * function last, in order to preserve the time stamps, if you're using
 * parallel subhandlers.
 */
Request.prototype.endTimer = function endTimer(subHandler) {
    var self = this;
    var timers = self.timers;

    var entry = self.lastHandler;
    if (entry) {
        if (subHandler) {
            entry.child[subHandler] = process.hrtime(entry.time);
        } else {
            entry.time = process.hrtime(entry.time);
        }
    }
