...
Request.prototype.startHandlerTimer = function startHandlerTimer(serverName,
                                                                 route,
                                                                 handlerName)
{
    var self = this;
    if (!self.dtraceId)
        self.dtraceId = dtrace.nextId();
    if (!self.timerMap)
        self.timerMap = {};

    self.timerMap[handlerName] = process.hrtime();
...
Request.prototype.endHandlerTimer = function endHandlerTimer(serverName,
                                                             route,
                                                             handlerName)
{
    var self = this;
    ...
    self.timerMap[handlerName] = process.hrtime(self.timerMap[handlerName]);
    self.timers.push({
        name: handlerName,
        time: self.timerMap[handlerName]
    });
